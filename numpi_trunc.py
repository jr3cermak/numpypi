...
def round_lastbits(x, bits=None):
    ...
    elif type(x) == float or type(x) == _numpy._numpy.float64:
...
def sin(*args, **kwargs):
    return round_lastbits(_numpy._numpy.sin(*args, **kwargs))

def cos(*args, **kwargs):
    return round_lastbits(_numpy._numpy.cos(*args, **kwargs))

def tan(*args, **kwargs):
    return round_lastbits(_numpy._numpy.tan(*args, **kwargs))

def arcsin(*args, **kwargs):
    return round_lastbits(_numpy._numpy.arcsin(*args, **kwargs))

def arccos(*args, **kwargs):
    return round_lastbits(_numpy._numpy.arccos(*args, **kwargs))

def arctan(*args, **kwargs):
    return round_lastbits(_numpy._numpy.arctan(*args, **kwargs))

def sqrt(*args, **kwargs):
    return round_lastbits(_numpy._numpy.sqrt(*args, **kwargs))

# These functions are needed because of a collision between python and numpy function names
def abs(*args, **kwargs):
    return _numpy._numpy.absolute(*args, **kwargs)
